  ...
  "Statement": [
    {
      ...
      "Condition": {
        "StringEquals": {
          "aws:ResourceTag/k8s.io/cluster-autoscaler/${aws_eks_cluster.main.id}": "owned"
        }
      }
    },
    {
...
# Create Kubernetes Role for EBS CSI Driver
resource "aws_iam_role" "ebs_csi_driver" {
  name = "${var.tenant}-${var.name}-eks-ebs-csi-role-${random_id.iam.hex}-${var.environment}"
...
# Attach Kubernetes Role Policy for EBS CSI Driver
